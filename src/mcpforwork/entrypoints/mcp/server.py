...
def apply_session() -> str:
    ...
    return (
        ...
        "report_apply_progress. Honor any portal quirks listed in the fill plan "
        "(from the source apply_playbook). Unknown screeners → resolve_field "
        "(ask the human on ask_user; save_form_answer). Obstacles "
        "(captcha/login/2FA) → pause for the human, then resume. Upload via "
        "get_asset_file when asked.\n"
...
def hunt_session() -> str:
    ...
    return (
        ...
        "1. Call hunt_plan to get per-source search URLs (each carries a `mode`).\n"
        "2. For each source, open its search_url in the user's browser. If mode is "
        "'search_box', the URL is the search PAGE — type the query into the on-page "
        "search box per result_hint (its query param does not filter); otherwise the "
        "URL is already the filtered results. Extract the postings (url, title, "
        "company_name, location, remote_scope, salary_text, description).\n"
